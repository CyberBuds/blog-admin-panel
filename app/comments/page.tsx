...
export default function CommentsPage() {
  ...
  // ✅ Fetch blogs
  const { data: blogsResponse, isLoading: isBlogsLoading } = useSWR<{
    data: Blog[];
    totalCount: number;
  }>(blogApi.getAll(), fetcher);


  // ✅ Fetch comments using blog's own tenantId
  const { data: pagedData, mutate, isLoading } = useSWR<{
    data: Comment[];
    totalCount: number;
    page: number;
    totalPages: number;
  ...
  selectedBlogId
  ? commentApi.getAdminComments(selectedBlogId)
  : null,
    fetcher
  );
  ...
  const toggleApproval = async (id: string, isApproved: boolean) => {
    try {
      if (isApproved) await commentApi.approve(id);
      else await commentApi.reject(id);
      mutate();
        ...
        <div className="flex items-center space-x-2">
          {!item.isApproved ? (
            <button onClick={() => toggleApproval(item.id, true)}
              className="p-2 hover:bg-emerald-50 dark:hover:bg-emerald-950/30 rounded-md transition-colors text-emerald-500" title="Approve">
              <CheckCircle className="h-4 w-4" />
            </button>
          ) : (
            <button onClick={() => toggleApproval(item.id, false)}
              className="p-2 hover:bg-amber-50 dark:hover:bg-amber-950/30 rounded-md transition-colors text-amber-500" title="Reject">
    ...
    <div className="space-y-6">
      ...
      {!selectedBlogId ? (
        <div className="flex justify-center p-8 text-muted-foreground">
          Select a blog post above to view comments.
        </div>
      ) : isLoading ? (
        ...
        <DataTable data={filteredData} columns={columns} onSearch={setSearch} />
      )}
