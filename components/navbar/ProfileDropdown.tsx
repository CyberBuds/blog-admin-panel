...
function TogglePill({ on }: { on: boolean }) {
  return (
    <span className={`relative inline-flex items-center w-9 h-5 rounded-full
        transition-colors duration-200
        ${on ? "bg-indigo-500" : "bg-gray-300 dark:bg-gray-600"}`}>
      <span className={`inline-block w-3.5 h-3.5 rounded-full bg-white shadow-sm
...
export default function ProfileDropdown() {
  ...
  // ✅ Real user data from your auth store
  const { user, logout } = useAuthStore();
  const { reset } = useTenantStore();

  const handleSignOut = () => {
    logout();       // clear auth store
    reset();        // clear tenant store
    router.push("/login");
  };
          ...
          <div className="flex items-center gap-3 px-4 py-4
            ...
            <div className="relative">
              ...
              <span className="absolute -bottom-0.5 -right-0.5 w-3.5 h-3.5
                  rounded-full bg-emerald-400 border-2
